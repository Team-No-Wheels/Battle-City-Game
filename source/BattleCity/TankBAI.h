...
{
	class TankBAI : public BasicTankAI
	{
		ATTRIBUTED_DECLARATIONS(TankBAI, BasicTankAI);

	public:

		/** TankBAI constructor.
		*/
		explicit TankBAI(const float speed = DEFAULT_SPEED, const int32_t bulletsNum = DEFAULT_BULLETS_NUM,
						 const int32_t armor = DEFAULT_ARMOR, const uint32_t probToShootInMov = DEFAULT_PROB_TO_SHOOT_IN_MOV);

		virtual ~TankBAI() = default;
	...
	ENTITY_FACTORY_DECLARATIONS(TankBAI);
}

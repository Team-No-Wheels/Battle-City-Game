...
namespace AnonymousEngine
{
	...
	void PowerUpSpawner::SpawnPowerUp()
	{
		...
		glm::vec4 position;
		position.x = static_cast<float>(generator.GetRangedRandom(4, 252));
		position.y = static_cast<float>(generator.GetRangedRandom(4, 236));
		position.z = 0;
		position.w = 0;
	...
	void PowerUpSpawner::AppendPrescribedAttributeNames(AnonymousEngine::Vector<std::string>& prescribedAttributeNames)
	{
